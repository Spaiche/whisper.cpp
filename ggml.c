        ...
        for (int i = 0; i < cgraph->n_nodes; i++)
        {
            ...
            case GGML_OP_CONV_1D_2S:
            {
                ...
                if (node->src0->type == GGML_TYPE_F16 &&
                    node->src1->type == GGML_TYPE_F32)
                {
                    cur = sizeof(ggml_fp16_t) * (nk * ggml_up32(node->src0->ne[1]) * node->src0->ne[2] +
                                                 (2 * (nk / 2) + node->src1->ne[0]) * node->src1->ne[1]);
                }
                else if (node->src0->type == GGML_TYPE_F32 &&
                         node->src1->type == GGML_TYPE_F32)
                {
                    cur = sizeof(float) * (nk * ggml_up32(node->src0->ne[1]) * node->src0->ne[2] +
                                           (2 * (nk / 2) + node->src1->ne[0]) * node->src1->ne[1]);
                }
                else
                {
...
void ggml_graph_print(const struct ggml_cgraph *cgraph)
{
    ...
    for (int i = 0; i < cgraph->n_nodes; i++)
    {
        ...
        GGML_PRINT(" - %3d: [ %6d, %6d, %6d] %16s %s (%3d) cpu = %7.3f / %7.3f ms, wall = %7.3f / %7.3f ms\n",
                   ...
                   GGML_OP_LABEL[node->op], node->is_param ? "x" : node->grad ? "g"
                                                                              : " ",
                   node->perf_runs,
